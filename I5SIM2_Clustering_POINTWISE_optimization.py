...
allTempJaccardSim = []
allTempNMI = []
totalResultFile = open('I5SIM2_POINTWISELSH_totalResult','a')
totalResultFile.write('(segment,numberHFs,numberRadius,AverageOfCCR, AverageOfJaccardSim, AverageOfNMI)')
for numberHFs in [2,15,25,51,101,151]:
	for numberRadius in [2,15,25,51,101,151]:
		tempCCR = []
		tempJacardSim = []
		tempNMI = []
		for c in range(1,6):
			...
			fileContainer = open('OUTPUT_Pointwise LSH Clustering Experiment -Optimization- ('+usedDataset[35:-4]+') HFs_'+ str(numberHFs)+'_R_'+str(numberRadius), 'a')
			fileContainer.write('\n')
