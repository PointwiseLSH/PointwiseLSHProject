...
fileContainer.write('\n# RUNID 47\ncrossDatasetApproximationNNResults = ')
fileContainer.write(str(allResults))
fileContainer.write('\ntheIndeces = ')
fileContainer.write(str(thresholdIndices))

crossDatasetApproximationNNResults = allResults
...
for precisionAt in range(1,100):#[1, 2, 10]:
	...
	for i, result in enumerate(crossDatasetApproximationNNResults):
		for j in range(precisionAt):
			if j >= len(result): # THIS TO AVOID ERROR IF THE RETRIEVED LIST SMALLLER THAT THE PRECISIONAT NEEDED
				break
			if result[j] in crossDatasetTrueClassification[mat.values()[3][i][0]-1]:
...
for i, result in enumerate(crossDatasetApproximationNNResults):
	intersectionCount_threshold1 += (len(list(set(result[:100]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(crossDatasetTrueClassification[mat.values()[3][i][0]-1])) # 100 IS THE NUMBER OF THE RELEVANT TRAJECTORIES (this is namely recall for each query)
	...
	if theIndeces[i][1] > 0: # if it is -1 means that no retrieved results at all.
		intersectionCount_threshold3 += (len(list(set(result[:theIndeces[i][1]]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))
	...
	if theIndeces[i][0] > 0: # if it is -1 means that no retrieved results at all.
		intersectionCount_threshold5 += (len(list(set(result[1:theIndeces[i][0]]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))
	...
	if len(result) > 0: # No need to checking the length as there a condition while generated the result for threshold to be at most 1
		retrievedCount_threshold1 += len(result[:100])
		# print 'retrievedCount_threshold1', retrievedCount_threshold1
		precisionCounter_threshold1 += (len(list(set(result[:100]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(result[:100]))	
	...
	if len(result[:theIndeces[i][1]]) > 0 and theIndeces[i][1] > 0:
		...
		precisionCounter_threshold3 += (len(list(set(result[:theIndeces[i][1]]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(result[:theIndeces[i][1]]))
	...
	if len(result[:theIndeces[i][0]]) > 0 and theIndeces[i][0] > 0:
		...
		precisionCounter_threshold5 += (len(list(set(result[:theIndeces[i][0]]).intersection(crossDatasetTrueClassification[mat.values()[3][i][0]-1]))))/float(len(result[:theIndeces[i][0]]))
...
# print '# of responces is : ', len(crossDatasetApproximationNNResults)
fileContainer.write('\n--------------------------------------------------------------------')
fileContainer.write('\n--------------------------------------------------------------------')
fileContainer.write('\n--------------------------------------------------------------------')
fileContainer.write('\nFor threshold 3 : ')
