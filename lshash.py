# lshash/lshash.py
# Copyright 2012 Kay Zhu (a.k.a He Zhu) and contributors (see CONTRIBUTORS.txt) (Slight changes have been done by Mohammed Almalki 2015 to perform Euclidean LSH)
#
...
class LSHash(object):
    ...
    def __init__(self, hash_size, input_dim, num_hashtables=1,
        ...
        self.loadFactor = 151#np.random.randn()#5#0.0077#
        self.offset = np.random.uniform(0, self.loadFactor, self.hash_size)#np.array([0,0])
        #self.offset = np.zeros(self.hash_size)
	...
	#print 'Here', self.uniform_planes #me
    ...
    def _init_uniform_planes(self):
        ...
        if self.matrices_filename:
            ...
            # self.uniform_planes = [np.array([[1,0], [0,1]])]
    ...
    def _hash(self, planes, input_point):
        ...
        try:
            ...
            #loadFactor = np.random.randn()
            #offset = np.random.uniform(0, loadFactor, self.hash_size)
            # print 'planes[0]', planes[0], '\n'
            # print 'input_point', input_point, '\n'
            # print 'np.dot(planes[0],input_point)', np.dot(planes[0],input_point), '\n'
            # print 'Hashing result is : ', (np.dot(planes[0],input_point), np.dot(planes[1],input_point), np.dot(planes[2],input_point))
            # print 'self.offset[0]', self.offset[0], '\n'
            # print 'self.loadFactor', self.loadFactor, '\n'
            # print '(np.dot(planes[0],input_point) + self.offset[0])', (np.dot(planes[0],input_point) + self.offset[0]), '\n'
            # print 'np.floor((np.dot(planes[0],input_point) + self.offset[0])/self.loadFactor)', np.floor((np.dot(planes[0],input_point) + self.offset[0])/self.loadFactor), '\n'
            projections = tuple(int(round(((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor))) for i in range(self.hash_size))#tuple([np.floor((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor) for i in range(self.hash_size)])
            # projections = tuple(int(np.floor(((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor))) for i in range(self.hash_size))#tuple([np.floor((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor) for i in range(self.hash_size)])
            # projections = tuple(int(np.ceil(((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor))) for i in range(self.hash_size))#tuple([np.floor((np.dot(planes[i],input_point) + self.offset[i])/self.loadFactor) for i in range(self.hash_size)])
            # print 'projections', projections, '\n'
            # print 'self.offset', self.offset, '\n'
            #return
			# tempValues = np.random.randn(3,len(input_point))
			# loadFactor = 01
			# offset = [1,1,1]
			# testing = tuple([((math.floor(np.dot(tempValues[i],testPoint)) + offset[i])/loadFactor) for i in range(len(offset))])
			...
			#print tuple([math.floor(i) for i in projections])
			#print type(projections)
			
			return projections #me 4
    ...
    def index(self, input_point, extra_data=None, loadF=None):
        ...
        # for i, table in enumerate(self.hash_tables):
            # table.append_val(self._hash(self.uniform_planes[i], input_point),
                             # value)
        #print '\n', self.hash_tables[0].storage[self.hash_tables[0].keys()[0]], input_point, '\n'
        # This because we in Pointwise LSH use only one hash table
        # print self.uniform_planes
        # print np.shape(self.uniform_planes)
        return self._hash(self.uniform_planes[0], input_point)		
							 
	#print 'self.hash_tables:', self.hash_tables #me2
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        #Stop these previous three lines and the next part of line to get the candidates list without measuring the distance for them
        #return list(candidates)[:num_results] if num_results else candidates me end of Thursday 
