...
allTempJaccardSim = []
allTempNMI = []
totalResultFile = open('CROSS_NoOverlapping_FFFFF_totalResult','a')
	...
	for numberHFs in [2,3,15,25,51,101,151,201]:
		for numberRadius in [2,3,15,25,51,101,151,201]:
			...
			for c in range(1,3):
				...
				fileContainer = open('Pointwise LSH Clustering Segmentation No Overlapping # - optimiziaton of points('+str(dimensionNumber)+') Experiment ('+usedDataset[35:-4]+') ', 'a')
				fileContainer.write('\n')
				...
				tempCCR.append(measurements.ccr(crossDatasetTrueClassification, clusteringResults))
				tempJacardSim.append(measurements.jaccardSim(crossDatasetTrueClassification, clusteringResults))
				tempNMI.append(measurements.NMI(crossDatasetTrueClassification, clusteringResults))
				...
				fileContainer.write('\n')
				fileContainer.write('The Correct Clustering Rate is : '+ str(measurements.ccr(crossDatasetTrueClassification, clusteringResults)))
				fileContainer.write('\n')
				fileContainer.write('The clustering Jaccard Similarity is : '+ str(measurements.jaccardSim(crossDatasetTrueClassification, clusteringResults)))
				print measurements.NMI(crossDatasetTrueClassification, clusteringResults)
				fileContainer.write('\nThe NMI is : '+ str(measurements.NMI(crossDatasetTrueClassification, clusteringResults)))
				fileContainer.write('\n--------------------------------------------------------------------')
			fileContainer.write('\n')
